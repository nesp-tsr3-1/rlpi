...
CalcLPI <- function(Species,
  ...
  AllPopulationsInterpolated <- data.frame(
    Binomial = character(),
    ID = integer(),
    year = integer(),
    popvalue = double(),
  ...
  for (I in 1:noSpecies) {
    ...
    for (J in 1:PopIDSize) {
      ...
      if ((length(PopN) >= DATA_LENGTH_MIN) & (AvgTimeBetweenPts < AVG_TIME_BETWEEN_PTS_MAX)) {
        ...
        # Save interpolated results into data frame
        AllPopulationsInterpolated <- rbind(AllPopulationsInterpolated, data.frame(
          Binomial = as.character(sNames[I, ]),
          ID = PopID[J],
          year = YearPopInt,
          popvalue = as.vector(PopNInt)
        ))
        ...
        for (K in StartYear:FinalYear) {
          if ((PopN[K - InitialYear + 1] != -1) & (PopN[K - InitialYear] !=
            ...
            PopLambda[JIndex, K - InitialYear + 1] <- PopN[K - InitialYear +
              1] - PopN[K - InitialYear]
          } else {
            PopLambda[JIndex, K - InitialYear + 1] <- -1
          }
        }
        JIndex <- JIndex + 1
      } else {
    ...
    write.table(
      AllPopulationsInterpolated,
      file.path(basedir, gsub(".txt", "_PopInt.txt", DatasetName)),
      sep = "\t",
      row.names = FALSE
    )
