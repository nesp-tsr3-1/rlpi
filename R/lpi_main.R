...
#' @param use_weightings_b - Whether to use the second level of weightings ('weightings_b') in the infile. Default=0
...
#' @param go_parallel - Should the code be executing in parallel, Default=FALSE
#'
...
LPIMain <- function(infile = "Infile.txt",
  ...
  # Set parallel mode
  `%op%` <- if (go_parallel) foreach::`%dopar%` else foreach::`%do%`
  ...
  # species_lambda_arrayTemp = matrix(data=NA,nrow=0,ncol=dsize)
  # Not quite sure why this needs to be transposed as it's empty and has the right dims, but it does
  species_lambda_array <- data.frame(NULL)
  speciesNamesArray <- data.frame(NULL)

  # Create empty (NAs) DTemp array
  dtemp_arrayTemp <- matrix(data = NA, nrow = NoFiles, ncol = dsize)
  dtemp_array <- data.frame(dtemp_arrayTemp)
  ...
  for (FileNo in 1:NoFiles) {
    ...
    # cat(dim(species_lambda_array), "\n")
    species_lambda_array <- plyr::rbind.fill(species_lambda_array, speciesLambda)
    ...
    dtemp_array[FileNo, 1:dim(DTemp)[2]] <- t(DTemp)
  }
  ...
  # write.table(dtemp_array, file="dtemp_array.txt")
  f_name <- file <- file.path(basedir, gsub(".txt", "_dtemp_array.txt", infile))
  cat("Saving DTemp Array to file: ", f_name, "\n")
  write.table(dtemp_array, f_name)

  dtemp_df <- data.frame(filenames = FileNames, dtemps = dtemp_array)
  ...
  # I = calculate_index(dtemp_array, fileindex, dsize, group, weightings, use_weightings)
  I <- calculate_index(dtemp_array, fileindex, dsize, group, weightings, use_weightings, use_weightings_b, weightings_b)
  ...
  if (ci_flag == 1) {
    ...
    BootI <- foreach::foreach(Loop = 1:boot_strap_size) %op% {
      ...
      # bootstrap_lpi(species_lambda_array, fileindex, dsize, group, weightings, use_weightings)
      bootstrap_lpi(species_lambda_array, fileindex, dsize, group, weightings, use_weightings, use_weightings_b, weightings_b, cap_lambdas)
      # sink()
    }
    cat("\n")
  ...
  if (lev_flag == 1) {
    ...
    for (i in 1:nrow(species_lambda_array)) {

      # species_lambda_array = species_lambda_array[-i, ]
      lev_I <- calc_leverage_lpi(species_lambda_array[-i, ], fileindex, dsize, group, weightings, use_weightings, use_weightings_b, weightings_b)
  ...
  if (switch_pt_flag == 1) {
    ...
    for (Loop in 1:boot_strap_size) {
      ...
      for (FileNo in 1:NoFiles) {
        ...
        dtemp_array[FileNo, 1:dim(t(DTemp))[1]] <- t(DTemp)
      }

      I <- calculate_index(dtemp_array, fileindex, dsize, group, weightings)
  ...
  if (plot_lpi) {
    if (ci_flag == 1) {
      ...
      plot_lpi(Ifinal, ref_year, plot_max, ci_flag, lower_ci, upper_ci)
    } else {
      # Plot the index
